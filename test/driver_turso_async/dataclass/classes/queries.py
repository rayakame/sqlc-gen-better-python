...
class Queries:
    ...
    def __init__(self, conn: turso.aio.Connection) -> None:
    ...
    @property
    def conn(self) -> turso.aio.Connection:
        """Connection object used to make queries.

        Returns:
            Connection object of type `turso.aio.Connection` used to make queries.
        """
        return self._conn
    ...
    async def insert_one_sqlite_type(
        ...
        *,
        ...
        await self._conn.execute(INSERT_ONE_SQLITE_TYPE, sql_args)
    ...
    async def get_one_date(self, *, id_: int, date_test: datetime.date) -> datetime.date | None:
        ...
        """
        row = await (await self._conn.execute(GET_ONE_DATE, (id_, date_test.isoformat()))).fetchone()
        if row is None:
            return None
    ...
    async def get_one_datetime(self, *, id_: int, datetime_test: datetime.datetime) -> datetime.datetime | None:
        ...
        """
        row = await (await self._conn.execute(GET_ONE_DATETIME, (id_, datetime_test.isoformat()))).fetchone()
        if row is None:
            return None
    ...
    async def get_one_timestamp(self, *, id_: int, timestamp_test: datetime.datetime) -> datetime.datetime | None:
        ...
        Returns:
            Result of type `datetime.datetime` fetched from the db. Will be `None` if not found.
        """
        row = await (await self._conn.execute(GET_ONE_TIMESTAMP, (id_, timestamp_test.isoformat()))).fetchone()
        if row is None:
            return None
        return datetime.datetime.fromisoformat(row[0])
    ...
    async def get_one_decimal(self, *, id_: int, decimal_test: decimal.Decimal) -> decimal.Decimal | None:
        ...
        """
        row = await (await self._conn.execute(GET_ONE_DECIMAL, (id_, str(decimal_test)))).fetchone()
        if row is None:
            return None
    ...
    async def get_one_blob(self, *, id_: int, blob_test: memoryview) -> memoryview | None:
        ...
        """
        row = await (await self._conn.execute(GET_ONE_BLOB, (id_, bytes(blob_test)))).fetchone()
        if row is None:
            return None
    ...
    async def update_result_one_sqlite_type(self, *, id_: int) -> turso.aio.Cursor:
    ...
    async def delete_result_one_sqlite_type(self, *, id_: int) -> turso.aio.Cursor:
