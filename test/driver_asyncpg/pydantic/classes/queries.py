...
class Queries:
    ...
    async def get_one_test_enum_type(self, *, id_: int) -> models.TestEnumType | None:
        ...
        """
        row = await self._conn.fetchrow(GET_ONE_TEST_ENUM_TYPE, id_)
        if row is None:
            return None
    ...
    async def get_one_test_enum_value(self, *, id_: int) -> enums.TestMood | None:
        ...
        """
        row = await self._conn.fetchrow(GET_ONE_TEST_ENUM_VALUE, id_)
        if row is None:
            return None
    ...
    async def delete_one_test_enum_type(self, *, id_: int) -> int:
        ...
        Returns:
            The number (`int`) of affected rows. This will be 0 for queries like `CREATE TABLE`.
        """
        r = await self._conn.execute(DELETE_ONE_TEST_ENUM_TYPE, id_)
        return int(n) if (p := r.split()) and (n := p[-1]).isdigit() else 0
