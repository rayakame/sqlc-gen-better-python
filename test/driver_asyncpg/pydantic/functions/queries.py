...
INSERT_ONE_TEST_ENUM_TYPE: typing.Final[str] = """-- name: InsertOneTestEnumType :exec
INSERT INTO test_enum_types (id, mood, maybe_mood)
VALUES ($1, $2, $3)
"""

GET_ONE_TEST_ENUM_TYPE: typing.Final[str] = """-- name: GetOneTestEnumType :one
SELECT id, mood, maybe_mood
FROM test_enum_types
WHERE id = $1
"""

GET_ONE_TEST_ENUM_VALUE: typing.Final[str] = """-- name: GetOneTestEnumValue :one
SELECT mood
FROM test_enum_types
WHERE id = $1
"""

GET_MANY_TEST_ENUM_TYPES: typing.Final[str] = """-- name: GetManyTestEnumTypes :many
SELECT id, mood, maybe_mood
FROM test_enum_types
ORDER BY id
"""

DELETE_ONE_TEST_ENUM_TYPE: typing.Final[str] = """-- name: DeleteOneTestEnumType :execrows
DELETE
FROM test_enum_types
WHERE id = $1
"""
...
async def get_one_test_enum_type(conn: ConnectionLike, *, id_: int) -> models.TestEnumType | None:
    ...
    """
    row = await conn.fetchrow(GET_ONE_TEST_ENUM_TYPE, id_)
    if row is None:
        return None
...
async def get_one_test_enum_value(conn: ConnectionLike, *, id_: int) -> enums.TestMood | None:
    ...
    """
    row = await conn.fetchrow(GET_ONE_TEST_ENUM_VALUE, id_)
    if row is None:
        return None
...
async def delete_one_test_enum_type(conn: ConnectionLike, *, id_: int) -> int:
    ...
    Returns:
        The number (`int`) of affected rows. This will be 0 for queries like `CREATE TABLE`.
    """
    r = await conn.execute(DELETE_ONE_TEST_ENUM_TYPE, id_)
    return int(n) if (p := r.split()) and (n := p[-1]).isdigit() else 0
