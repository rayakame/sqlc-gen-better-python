...
class Queries:
    ...
    async def get_all_embedded_test_postgres_type(self) -> GetAllEmbeddedTestPostgresTypeRow | None:
        row = await self._conn.fetchrow(GET_ALL_EMBEDDED_TEST_POSTGRES_TYPE)
        if row is None:
            return None
    ...
    async def get_embedded_test_postgres_type(self) -> GetEmbeddedTestPostgresTypeRow | None:
        row = await self._conn.fetchrow(GET_EMBEDDED_TEST_POSTGRES_TYPE)
        if row is None:
            return None
    ...
    async def get_many_test_bytea_postgres_type(self) -> collections.abc.Sequence[memoryview]:
    ...
    async def get_many_test_postgres_type(self) -> collections.abc.Sequence[models.TestPostgresType]:
    ...
    async def get_many_test_timestamp_postgres_type(self) -> collections.abc.Sequence[datetime.datetime]:
    ...
    async def get_one_inner_test_postgres_type(self) -> models.TestInnerPostgresType | None:
        row = await self._conn.fetchrow(GET_ONE_INNER_TEST_POSTGRES_TYPE)
        if row is None:
            return None
