...
from test.driver_asyncpg.omit_tc.functions import enums
from test.driver_asyncpg.omit_tc.functions import models
...
async def insert_enum_override(conn: ConnectionLike, *, id_: int, mood_test: str) -> None:
    ...
    Args:
        ...
        mood_test: str.
    """
    await conn.execute(INSERT_ENUM_OVERRIDE, id_, enums.TestMood(mood_test))
...
async def get_enum_override_mood(conn: ConnectionLike, *, id_: int) -> str | None:
    ...
    """
    row = await conn.fetchrow(GET_ENUM_OVERRIDE_MOOD, id_)
    if row is None:
        return None
    return str(row[0])
