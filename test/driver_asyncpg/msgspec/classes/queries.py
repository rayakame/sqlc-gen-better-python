...
__all__: collections.abc.Sequence[str] = (
    ...
    "GetFieldNamingDuplicateColumnsRow",
    "Queries",
    "QueryResults",
    "TestCopyFromParams",
)
...
if typing.TYPE_CHECKING:
    ...
    QueryResultsArgsType: typing.TypeAlias = int | float | str | memoryview | decimal.Decimal | uuid.UUID | datetime.date | datetime.time | datetime.datetime | datetime.timedelta | collections.abc.Sequence[typing.Any] | None
...
class GetFieldNamingDuplicateColumnsRow(msgspec.Struct):
    """Model representing GetFieldNamingDuplicateColumnsRow.
...
SELECT id, outputs, groups
FROM test_field_namings
WHERE id = $1 LIMIT 1
"""

GET_FIELD_NAMING_DUPLICATE_COLUMNS: typing.Final[str] = """-- name: GetFieldNamingDuplicateColumns :one
...
UPDATE_FIELD_NAMING_MIXED_PARAMS: typing.Final[str] = """-- name: UpdateFieldNamingMixedParams :exec
...
class Queries:
    ...
    async def get_field_naming(self, *, id_: int) -> models.TestFieldNaming | None:
        ...
        if row is None:
            return None
        return models.TestFieldNaming(id=row[0], outputs=row[1], groups=row[2])

    async def get_field_naming_duplicate_columns(self, *, id_: int) -> GetFieldNamingDuplicateColumnsRow | None:
        """Fetch one from the db using the SQL query with `name: GetFieldNamingDuplicateColumns :one`.
        ...
        Returns:
        GetFieldNamingDuplicateColumnsRow -- Result fetched from the db. Will be `None` if not found.
        """
        row = await self._conn.fetchrow(GET_FIELD_NAMING_DUPLICATE_COLUMNS, id_)
        if row is None:
            return None
        return GetFieldNamingDuplicateColumnsRow(outputs=row[0], outputs_2=row[1])
    ...
    async def update_field_naming_mixed_params(self, *, id_: int, outputs: str, outputs_2: str) -> None:
        """Execute SQL query with `name: UpdateFieldNamingMixedParams :exec`.
        ...
        outputs_2 -- str.
        """
        await self._conn.execute(UPDATE_FIELD_NAMING_MIXED_PARAMS, id_, outputs, outputs_2)
