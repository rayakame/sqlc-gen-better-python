...
SELECT * FROM test_slice WHERE id IN (sqlc.slice('ids')) AND name = sqlc.arg(wanted) AND (note IS NULL OR name = sqlc.arg(wanted)) AND id IN (sqlc.slice('ids')) ORDER BY id;
...
-- name: GetNamedSliceRow :one
SELECT * FROM test_slice WHERE id IN (sqlc.slice('ids')) AND name = sqlc.arg(wanted) ORDER BY id LIMIT 1;
