...
def insert_enum_override(conn: ConnectionLike, *, id_: int, mood_test: str) -> None:
    ...
    mood_test -- str.
    """
    conn.execute(INSERT_ENUM_OVERRIDE, {"p1": id_, "p2": enums.TestMood(mood_test)})


def get_enum_override_mood(conn: ConnectionLike, *, id_: int) -> str | None:
    ...
    """
    row = conn.execute(GET_ENUM_OVERRIDE_MOOD, {"p1": id_}).fetchone()
    if row is None:
        return None
    return str(row[0])
...
def count_enum_override_by_moods(conn: ConnectionLike, *, dollar_1: collections.abc.Sequence[enums.TestMood]) -> int | None:
    ...
    """
    row = conn.execute(COUNT_ENUM_OVERRIDE_BY_MOODS, {"p1": list(dollar_1)}).fetchone()
    if row is None:
        return None
    return row[0]
