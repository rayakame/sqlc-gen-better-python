...
class QueriesEnumOverride:
    ...
    async def insert_enum_override(self, *, id_: int, mood_test: str) -> None:
        ...
        mood_test -- str.
        """
        await self._conn.execute(INSERT_ENUM_OVERRIDE, {"p1": id_, "p2": enums.TestMood(mood_test)})
    ...
    async def get_enum_override_mood(self, *, id_: int) -> str | None:
        ...
        """
        row = await (await self._conn.execute(GET_ENUM_OVERRIDE_MOOD, {"p1": id_})).fetchone()
        if row is None:
            return None
        return str(row[0])
