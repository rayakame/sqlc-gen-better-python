...
| `omit_kwargs_limit`              | integer        | no       | If set to a non-negative value, queries with more parameters than the limit require keyword arguments (e.g. `0` makes every query with parameters keyword-only). If unset or negative, parameters are regular positional-or-keyword arguments. |
