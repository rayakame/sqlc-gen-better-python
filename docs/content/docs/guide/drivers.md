...
{{< callout type="warning" >}}
  Both SQLite drivers require `detect_types=PARSE_DECLTYPES` on the connection.
  The generated code registers adapters and converters (for dates, decimals,
  booleans, and blobs) that only run when declared-type parsing is enabled. See
